...
from .preprocess import preprocess_sceneweaver_dataset, write_dataset_catalog
from .renderable import build_sceneweaver_renderables, write_renderable_catalog
...
def _default_sceneweaver_import_subset(source: Path) -> str:
    parent_name = source.parent.name
    grandparent_name = source.parent.parent.name

    if parent_name:
        if re.fullmatch(r"\d{2}-\d{2}-\d{2}", parent_name) and re.fullmatch(
            r"\d{4}-\d{2}-\d{2}", grandparent_name
        ):
...
def _has_iteration_files(path: Path) -> bool:
    return any(path.glob("record_files/scene_*.blend")) and any(
        path.glob("record_scene/layout_*.json")
    )


def _is_sceneweaver_run_dir(path: Path) -> bool:
    return (
        path.is_dir()
        and (path / "args.json").exists()
        and (path / "roominfo.json").exists()
        and (path / "record_files").is_dir()
        and (path / "record_scene").is_dir()
        and _has_iteration_files(path)
    )


def _has_sceneweaver_run_ancestor(path: Path, stop_at: Path) -> bool:
    for parent in path.parents:
        if parent == stop_at:
            return False
        if _is_sceneweaver_run_dir(parent):
            return True
    return False


def _discover_local_sceneweaver_import_groups(
    source: Path,
    ...
    if _is_sceneweaver_run_dir(resolved):
        return [(_default_sceneweaver_import_subset(resolved), [resolved])], []

    if not resolved.is_dir():
        raise SystemExit(
            "Expected a SceneWeaver run directory or a root containing multiple run outputs."
        )

    direct_run_dirs = [
        path.resolve() for path in sorted(resolved.iterdir()) if _is_sceneweaver_run_dir(path)
    ]
    if direct_run_dirs:
        return [(_default_sceneweaver_import_subset(resolved), direct_run_dirs)], []

    grouped: dict[str, list[Path]] = {}
    skipped: list[dict[str, object]] = []
    candidate_dirs = [path.resolve() for path in sorted(resolved.rglob("*")) if path.is_dir()]
    for candidate in candidate_dirs:
        if _has_sceneweaver_run_ancestor(candidate, resolved):
            continue
        if _is_sceneweaver_run_dir(candidate):
            subset_name = _default_sceneweaver_import_subset(candidate)
            grouped.setdefault(subset_name, []).append(candidate)
            continue
        if (candidate / "args.json").exists() or (candidate / "roominfo.json").exists():
            skipped.append(
                {
    ...
    if grouped:
        return [
            (subset_name, scene_dirs) for subset_name, scene_dirs in sorted(grouped.items())
        ], skipped

    raise SystemExit(
        "Did not find any valid SceneWeaver run directories under "
        f"{source}. A valid run needs args.json, roominfo.json, record_files/, "
        "record_scene/, scene_*.blend, and layout_*.json."
    )
...
def _sceneweaver_preview_rebuild_reason(*, requested: bool) -> str | None:
    if requested:
        return "requested"

    preprocessed_index = PREPROCESSED_ROOT / "sceneweaver" / "index.json"
    if not preprocessed_index.exists():
        return "missing_preprocessed_index"

    renderable_index = RENDERABLE_ROOT / "sceneweaver" / "index.json"
...
def import_local_sceneweaver_output(
    *,
    source: Path,
    subset: str | None,
    ...
    spec = DATASETS["sceneweaver"]
    resolved_source = source.resolve()
    discovered_groups, discovery_skipped = _discover_local_sceneweaver_import_groups(
        resolved_source
    )
    ...
    preview_rebuild_reason = _sceneweaver_preview_rebuild_reason(requested=build_preview)
